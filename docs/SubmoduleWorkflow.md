...
* Tells Git to ignore working tree and pointer changes for that submodule in status and diff output.
* Effect: `git status` in the root repo usually stops showing “modified: bundles/bundle-rantamuta” every time the bundle commit advances locally.
* Important: if you explicitly stage the submodule path with `git add bundles/bundle-rantamuta`, the new gitlink pointer is still recorded normally in the next root commit. Nothing about commit storage changes.
...
1. When you want an integration checkpoint, intentionally update root pointer:
