...
NPC actions MUST use the same dispatcher entrypoint and pipeline used by players for all world mutation and render output.
NPC-only commands/instructions are allowed, but they MUST execute through that same pipeline.

There is no alternate execution path for NPC-caused side effects.

## Non-Negotiable Contract (v1)

NPC scripts are decision producers, not mutation executors.

Allowed in NPC scripts:

- read world state required for deterministic decision logic,
- read/write NPC-local ephemeral runtime fields (for example `npc.__runtime`) when not authoritative or persisted world state,
- emit intent through shared NPC/player command dispatch entrypoints.

Forbidden in NPC scripts:

- direct world/entity mutation (for example direct `moveTo(...)`, direct inventory/equipment/effect/attribute writes, direct room/area/player/item authoritative state writes),
- direct mutation of persisted player progression/state,
- direct combat damage/healing application,
- bypass of command Capture/Plan/Commit for diegetic actions,
- direct transport/broadcast output for actor-authored diegetic actions.

Unsupported mutation rule:

1. If required behavior cannot be represented by existing planner+mutator operations, command execution MUST fail with a structured error envelope.
2. No direct-mutation fallback path may be used.
3. Implementation MUST stop pending addition of the required mutation instruction/capability.
4. For this failure class, the error code MUST be `UNSUPPORTED_MUTATION_OP`.
...
Direct `Broadcast.*` calls from NPC scripts are prohibited for actor-authored diegetic actions.

Semantic events and other render instructions are the allowed delivery surface.

### Actor Feedback

v1 dispatcher success contract is:

```js
{ ok: true }
```

Structured actor-feedback payloads are deferred and are not required in v1.

Illustrative deferred shape:
...
- NPCs may receive structured feedback for decision logic.
...
- Planner assertions are secondary safety checks and MUST NOT replace Capture gating.
...
These will be defined in a later scheduling/autonomy or combat architecture document.
...
3. Replace Tomo direct speech output with dispatcher-based calls.
4. Remove direct mutation paths from Tomo behavior by routing through command planner+commit.
5. Add structured feedback consumption in Tomo AI if required.

Tomo is a disposable test-bed and may be rewritten entirely.

## Deferred Notes

1. Actor-feedback payloads beyond `{ ok: true }` are deferred.
2. Phase-0 command lookup hiding for NPC-only commands is deferred; v1 uses Capture veto (this may expose command existence to players via `ACTOR_KIND_FORBIDDEN` text).
3. Runtime hard-fail guardrails for known direct-mutation helper usage in NPC scripts are deferred.
