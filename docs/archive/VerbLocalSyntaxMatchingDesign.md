# Verb-Local Syntax Matching Implementation Design Document
...
### Recommended changes to the command architecture document
...
`LIVING` is an entity-bearing slot kind that behaves like `ENTITY` but only matches entities where `.isNpc === true`.
